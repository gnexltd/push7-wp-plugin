...
class Push7
{
    ...
    public function __construct()
    {
        ...
        add_filter("plugin_action_links_" . PUSH7_BASE_NAME, [
            $this,
            "add_setting_link",
        ]);
    }
    ...
    {
        return array_merge(get_post_types(["_builtin" => false]), [
            "post" => "post",
        ]);
    }
    ...
    public function init()
    {
        ...
        foreach (get_categories() as $category) {
            ...
            /* 後方互換性の維持コード */
            if (
                get_option($opt, null) === "1" ||
                is_null(get_option($opt, null))
            ) {
    ...
    public function add_setting_link($links)
    {
        return array_merge($links, [
            '<a href="' . menu_page_url("push7", false) . '">設定</a>',
        ]);
    }
}
